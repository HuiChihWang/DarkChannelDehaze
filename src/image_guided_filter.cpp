...
void CImageGuidedFilter::SetGuidedImage(const cv::Mat& matGuidedImage) {
	...
	if (matGuidedImage.channels() == 3) {
		cv::cvtColor(matGuided32F, matGuided32F, cv::COLOR_BGR2GRAY);
	}
...
void CImageGuidedFilter::CalculateMean() {
	...
	cv::Mat matKernel = cv::Mat::ones(iPatchSize, iPatchSize, CV_64F) / static_cast<double>(iPatchSize * iPatchSize);
...
void CImageGuidedFilter::CalculatePatchVariance() {
	int iPatchSize = m_tSetting.iPatchRadius * 2 + 1;
	cv::Mat matKernel = cv::Mat::ones(iPatchSize, iPatchSize, CV_64F) / (iPatchSize * iPatchSize);
